  ...
  void Price_check() {
    ...
    if (quantitys.length > 1) {
      for (int i = 0; i <= quantitys.length - 1; i++) {
        _price_uant += (products[quantitys[i].numberProduct].pricetext *
            quantitys[i].quantity);
        print("размер массива  больше 1");
      }
    } else {
      _price_uant = products[quantitys[0].numberProduct].pricetext *
          quantitys[0].quantity;
      print("размер массива  меньше 1");
    }
  }

  void check_price_poz() {
    if (quantitys.length = null) {}
  }
